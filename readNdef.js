...
async function selectFile( reader, fileType ) {
		...
		console.log( 'Sending:', apdu.toString( 'hex' ).toUpperCase() );
		const res = await reader.transmit( apdu, 2 );
		console.log( 'Received:', res.toString( 'hex' ).toUpperCase() );

	}

	async function attemptRead( reader ) {
		...
		console.log( 'Sending:', apdu.toString( 'hex' ).toUpperCase() );
		const response = await reader.transmit( apdu, 300 );
		console.log( 'Received', response.toString( 'hex' ).toUpperCase() );
...
module.exports = async function readNdef ( reader ) {
